...
class Solution {
    public int findRadius(int[] houses, int[] heaters) {
        ...
        for(int house: houses){
            ...
            int dL = index - 1 >= 0 ? house - heaters[index - 1] : Integer.MAX_VALUE;
            int dR = index < heaters.length ? heaters[index] - house : Integer.MAX_VALUE;
            res = Math.max(res, Math.min(dL, dR));
        }
